...
class TestThreadDB(unittest.TestCase):
    ...
    def test_general(self):
        ...
        randbers = [randint(21, 51)] * 3          # 1 random numbers between 21 and 50. 3 times in list
        threads = []
...
#class HelpProcessTest:
#    """ Because multiprocess can not target methods in an unittest.TestCase class I had to add this class """
...
class TestProcessDB(unittest.TestCase):
    ...
    def set_value(self, key, val):
        ...
        for _ in range(TestProcessDB.reps):
            self.sync_db.set_value(key, val)

    def get_value(self, key):
        """ Test get_value method """
        for _ in range(TestProcessDB.reps):
            self.sync_db.get_value(key), key * 100

    def set_value_special(self, key):
        """ Test set_value method based on a previous value """
        for _ in range(TestProcessDB.reps):
            self.sync_db._set_value_testing(key)

    def get_value_exists(self, key):
        """ Test get_value method just that returns some int """
        for _ in range(TestProcessDB.reps):
            self.sync_db.get_value(key), int
    ...
    def setUp(self):
        ...
        with open(TestProcessDB.test_fname, "wb") as f:
            pickle.dump(self.test_dict, f)
        self.sync_db = SyncDataBase(0, TestProcessDB.test_fname)
    ...
    def test_read_simple(self):
        """ Tests reading without competition """
        proc = multiprocessing.Process(target=self.get_value, name="proc_ws", args=(40, 4002))
        proc.start()
        proc.join()
        self.assertEqual(self.get_database_dict(), self.test_dict)
